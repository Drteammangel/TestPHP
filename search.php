...
if (!isset($item)) {
	...
	$sql = "select * from user_info where user_id='$item' or user_sex='$item' or user_name like '%" . $item . "%' or user_comment like '%" . $item . "%' or user_degree like '%" . $item . "%' or user_grade like '%" . $item . "%' order by user_id limit $offset, $pageSize";
	$sql1 = "select count(user_id) as count from user_info where user_id='$item' or user_sex='$item' or user_name like '%" . $item . "%' or user_comment like '%" . $item . "%' or user_degree like '%" . $item . "%' or user_grade like '%" . $item . "%'";
}
...
<div style="width: 80%; margin-left: 10%; margin-top: 3%;">
	<div class="panel panel-default">
		<div class="panel-heading">
			<form name="searchItem" action="search.php">
				会员后台管理
            <span style="position: absolute; right: 12%">
                    <input type="text" name="item" id="item">
                    <label for="item"><input type="button" class="btn btn-sm" value="搜索" onclick="search()"></label>
            </span>
			</form>
		</div>
		<div class="panel-body">
			<form action="delete.php" method="get" name="list">
				...
				//分页代码
				//计算留言总数
				$count_result = mysql_query($sql1, $con) or die(mysql_error());
